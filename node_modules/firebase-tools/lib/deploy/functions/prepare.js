...
exports.DECLARATIVE_SECURITY_ETAG_LABEL = exports.EVENTARC_SOURCE_ENV = void 0;
...
exports.getExecutablePaths = getExecutablePaths;
exports.stripStaleDartBuildIgnore = stripStaleDartBuildIgnore;
...
exports.checkKitForGen1 = checkKitForGen1;
...
const dart_1 = require("./runtimes/dart");
const features_1 = require("./runtimes/dart/features");
...
exports.DECLARATIVE_SECURITY_ETAG_LABEL = "firebase-declarative-security-etag";
async function discoverSecurityDetails(codebase, want, have, projectId, filters) {
    const requiredRoles = want.requiredRoles;
    const existingManagedSA = backend.findEndpoint(have, (e) => typeof e.serviceAccount === "string" && e.serviceAccount.startsWith("firebase-fn-"))?.serviceAccount ?? undefined;
    const haveRolesEtag = backend.findEndpoint(have, (e) => !!e.labels?.[exports.DECLARATIVE_SECURITY_ETAG_LABEL])?.labels?.[exports.DECLARATIVE_SECURITY_ETAG_LABEL];
    const isPartiallyFiltered = (0, functionsDeployHelper_1.isCodebasePartiallyFiltered)(codebase, filters);
    ...
    for (const endpoint of backend.allEndpoints(want)) {
        ...
        endpoint.labels[exports.DECLARATIVE_SECURITY_ETAG_LABEL] = newEtag;
    }
...
async function prepare(context, options, payload) {
    ...
    for (const [codebase, wantBuild] of Object.entries(wantBuilds)) {
        ...
        const firebaseEnvs = functionsEnv.loadFirebaseEnvs(firebaseConfig, projectId, (0, projectConfig_1.isKitConfig)(config) ? codebase : undefined);
        ...
        const configDir = (0, projectConfig_1.resolveConfigDir)(localCfg, codebase);
        if (configDir) {
    ...
    for (const [codebase, wantBackend] of Object.entries(wantBackends)) {
        ...
        if (backend.someEndpoint(wantBackend, (e) => e.platform === "gcfv2" || e.platform === "run")) {
            ...
            const uploadCfg = await stripStaleDartBuildIgnore(wantBuilds[codebase].runtime, sourceDir, localCfg);
            const packagedSource = await (0, prepareFunctionsUpload_1.prepareFunctionsUpload)(options.config.projectDir, sourceDir, uploadCfg, [...schPathSet], undefined, { exportType, executablePaths });
...
async function loadCodebases(config, options, firebaseConfig, runtimeConfig, filters) {
    ...
    for (const codebase of codebases) {
        ...
        const configDir = (0, projectConfig_1.resolveConfigDir)(codebaseConfig, codebase);
        if (configDir) {
            userEnvOpt.configDir = options.config.path(configDir);
        }
        const userEnvs = functionsEnv.loadUserEnvs(userEnvOpt);
        const firebaseEnvs = functionsEnv.loadFirebaseEnvs(firebaseConfig, projectId, (0, projectConfig_1.isKitConfig)(codebaseConfig) ? codebase : undefined);
        ...
        if ((0, projectConfig_1.isKitConfig)(codebaseConfig)) {
...
async function getExecutablePaths(runtime, sourceDir) {
    if (!supported.runtimeIsLanguage(runtime, "dart")) {
        return [];
    }
    const features = await features_1.DartVersionFeatures.detect(sourceDir);
    return features.isNativeAssetsAvailable ? [dart_1.DART_BUNDLE_EXECUTABLE_PATH] : [dart_1.DART_COMPILE_EXE_PATH];
}
async function stripStaleDartBuildIgnore(runtime, sourceDir, localCfg) {
    if (!supported.runtimeIsLanguage(runtime, "dart") ||
...
function checkKitForGen1(localCfg, wantBuild) {
    if ((0, projectConfig_1.isKitConfig)(localCfg) &&
        Object.values(wantBuild.endpoints).some((e) => e.platform === "gcfv1")) {
        throw new error_1.FirebaseError(`Function kit "${localCfg.kit}" contains gen1 functions, which are not supported in kits. Please remove this kit or upgrade these functions to gen2.`);
    }
}

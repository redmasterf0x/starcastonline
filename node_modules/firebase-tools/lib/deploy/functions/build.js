...
exports.AllIngressSettings = exports.AllVpcEgressSettings = exports.AllFunctionsPlatforms = exports.SECRET_REF_LONG_RE = exports.SECRET_REF_SHORT_RE = exports.GCP_SECRET_ID_PATTERN = exports.SECRET_REF_PREFIX = exports.REGION_TBD = void 0;
exports.empty = empty;
exports.of = of;
...
exports.applyKitSecretRefPrefix = applyKitSecretRefPrefix;
exports.applyEndpointPrefix = applyEndpointPrefix;
exports.applyEnvSecretBindingsToParams = applyEnvSecretBindingsToParams;
exports.applyEnvSecretBindingsToBuild = applyEnvSecretBindingsToBuild;
...
exports.SECRET_REF_PREFIX = "FIREBASE_SECRET_REF_";
const GCP_PROJECT_ID_OR_NUM_PATTERN = "((?:\\d+)|(?:[a-z][-a-z0-9]{4,28}[a-z0-9]))";
exports.GCP_SECRET_ID_PATTERN = "([a-zA-Z0-9-_]+)";
exports.SECRET_REF_SHORT_RE = new RegExp("^" +
    exports.GCP_SECRET_ID_PATTERN +
    "(?:[:#@]" +
    "([a-z0-9-_]+)" +
    ")?$");
exports.SECRET_REF_LONG_RE = new RegExp("^" +
    "projects/" +
    GCP_PROJECT_ID_OR_NUM_PATTERN +
    "/secrets/" +
    exports.GCP_SECRET_ID_PATTERN +
    "(?:(?:/versions/|:|#|@)" +
    "([a-z0-9-_]+)" +
...
function applyKitSecretRefPrefix(build, instanceId) {
    const kitPrefix = (0, projectConfig_1.addKitPrefix)(instanceId);
...
function applyEndpointPrefix(build, prefix) {
    if (!prefix) {
        return;
    }
    const newEndpoints = {};
...
function applyEnvSecretBindingsToParams(params, envSecrets) {
...
async function applyEnvSecretBindingsToBuild(build, envSecrets) {
    if (envSecrets.empty) {
        return;
    }
    logger_1.logger.debug(`Attempting to merge .env secret bindings ${JSON.stringify(envSecrets)} into declared secrets...)}`);
    for (const endpointName of Object.keys(build.endpoints)) {
        logger_1.logger.debug(`${endpointName} declared secrets: ${JSON.stringify(build.endpoints[endpointName].secretEnvironmentVariables)}`);
    }
    applyEnvSecretBindingsToParams(build.params, envSecrets);
    const projectNumberToId = new Map();
    for (const key of Object.keys(envSecrets)) {
        ...
        for (const endpointName of Object.keys(build.endpoints)) {
            ...
            if (projectId && projectId !== endpoint.project) {
                throw new error_1.FirebaseError(`Secret binding ${key} referenced unsupported cross-project secret in '${projectId}'`);
            }
            let notFound = true;
            for (const envVar of endpoint.secretEnvironmentVariables ?? []) {
                if (envVar.key === key) {
                    ...
                    logger_1.logger.debug(`Merged secret: ${JSON.stringify(envVar)}`);
                }
            }
            if (notFound) {
                logger_1.logger.warn(`.env files contain a secret binding ${key} which has not been configured as a secret param via defineSecret().`);
            }
        }
    }
}
function parseSecretRef(ref) {
    const shortMatch = exports.SECRET_REF_SHORT_RE.exec(ref);
    if (shortMatch) {
        const output = {
        ...
        if (shortMatch[2] !== undefined) {
            ...
            if (ref.includes("#") || ref.includes("@")) {
                throw new error_1.FirebaseError(`Malformed secret binding '${ref}'; secret versions are specified with ':'`);
            }
        }
        return output;
    }
    const longMatch = exports.SECRET_REF_LONG_RE.exec(ref);
    if (longMatch) {
        const output = {
    ...
    throw new error_1.FirebaseError(`Unknown format for secret binding '${ref}'`);
}

...
exports.isSecretParam = isSecretParam;
...
async function resolveParams(opts) {
    const { params, firebaseConfig, userEnvs, codebase, nonInteractive = false, force = false, isEmulator = false, } = opts;
    ...
    if (!isEmulator) {
        for (const param of needSecret) {
            secretRefs[param.name] = await ensureSecret(param, firebaseConfig.projectId, nonInteractive, force);
        }
    }
    ...
    if (needPrompt.length > 0) {
        logger_1.logger.info(`Prompting for parameters for codebase ${clc.bold(codebase)}:`);
    }
...
async function ensureSecret(secretParam, projectId, nonInteractive, force) {
    ...
    if (!metadata.secret) {
        if (nonInteractive) {
            throw new error_1.FirebaseError(`In non-interactive mode but have no value for the secret ${secretParam.name}: ${resourceId}\n\n` +
        ...
        if (experiments.isEnabled("secretEnvParams") && typeof secretParam.resourceId === "undefined") {
            if (force) {
                logger_1.logger.info(`--force: Using default resource ID for secret ${secretParam.name}`);
                secretParam.resourceId = secretParam.name;
            }
            else {
                secretParam.resourceId = await (0, prompt_1.input)({
        ...
        logger_1.logger.info(`\n${clc.bold(label)}: ${(await (0, marked_1.marked)(desc)).trim()}`);
        ...
        await secretManager.createSecret(projectId, resourceId, (0, secretManager_1.labels)());
    ...
    if (experiments.isEnabled("secretEnvParams")) {
        if (!secretParam.inLocalEnvironment && secretAlreadyExisted) {
            logger_1.logger.info(`Onetime (firebase-tools x.y.z+): storing a reference to existing secret ${secretParam.name}=${secretRefString} in .env files.`);
        }
    }
...
async function promptParam(param, projectId, resolvedDefault) {
    ...
    if (param.description) {
        logger_1.logger.info(`\n${clc.bold(label)}: ${(await (0, marked_1.marked)(param.description)).trim()}`);
    }
    else {
        logger_1.logger.info("");
    }
...
async function promptList(param, projectId, resolvedDefault) {
    ...
    if (isSelectInput(param.input)) {
        ...
        return promptSelectMultiple(`Select values for ${label}:`, param.input, resolvedDefault, param.input.multiSelect.nonEmpty, (res) => res);
    }
    else if (isTextInput(param.input)) {
        const delimiter = param.delimiter ? param.delimiter : ",";
        return promptText(`Enter a list of strings (delimiter: ${delimiter}) for ${label}:`, param.input, resolvedDefault, param.input.text.nonEmpty, (res) => {
...
async function promptBooleanParam(param, resolvedDefault) {
    ...
    const label = param.label || param.name;
    if (isSelectInput(param.input)) {
        return promptSelect(`Select a value for ${label}:`, param.input, resolvedDefault, isTruthyInput);
    }
    else if (isMultiSelectInput(param.input)) {
        throw new error_1.FirebaseError("Non-list params cannot have multi selector inputs");
    }
    else if (isTextInput(param.input)) {
        return promptText(`Enter a boolean value for ${label}:`, param.input, resolvedDefault, false, isTruthyInput);
    }
    else if (isResourceInput(param.input)) {
...
async function promptStringParam(param, projectId, resolvedDefault) {
    ...
    if (isResourceInput(param.input)) {
        return promptResourceString(`Select a value for ${label}:`, param.input, projectId, resolvedDefault);
    }
    else if (isMultiSelectInput(param.input)) {
        throw new error_1.FirebaseError("Non-list params cannot have multi selector inputs");
    }
    else if (isSelectInput(param.input)) {
        return promptSelect(`Select a value for ${label}:`, param.input, resolvedDefault, (res) => res);
    }
    else if (isTextInput(param.input)) {
        return promptText(`Enter a string value for ${label}:`, param.input, resolvedDefault, param.input.text.nonEmpty, (res) => res);
    }
    else {
        (0, functional_1.assertExhaustive)(param.input);
    }
...
async function promptIntParam(param, resolvedDefault) {
    ...
    const label = param.label || param.name;
    if (isSelectInput(param.input)) {
        return promptSelect(`Select a value for ${label}:`, param.input, resolvedDefault, (res) => {
            if (isNaN(+res)) {
        ...
        return promptText(`Enter an integer value for ${label}:`, param.input, resolvedDefault, param.input.text.nonEmpty, (res) => {
            if (isNaN(+res)) {
                return { message: `"${res}" could not be converted to a number.` };
            }
            if (res.includes(".")) {
...
async function promptResourceString(prompt, input, projectId, resolvedDefault) {
    ...
    switch (input.resource.type) {
        ...
        default:
            ...
            return promptText(prompt, { text: {} }, resolvedDefault, true, (res) => res);
    }
}
async function promptResourceStrings(prompt, input, projectId, enforceNonEmpty) {
    const notFound = new error_1.FirebaseError(`No instances of ${input.resource.type} found.`);
    switch (input.resource.type) {
        case "storage.googleapis.com/Bucket":
            const buckets = (await (0, storage_1.listBuckets)(projectId)).map((b) => b.name);
            ...
            return promptSelectMultiple(prompt, forgedInput, undefined, enforceNonEmpty, (res) => res);
        default:
            logger_1.logger.warn(`Warning: unknown resource type ${input.resource.type}; defaulting to raw text input...`);
            return promptText(prompt, { text: {} }, undefined, enforceNonEmpty, (res) => res.split(","));
    }
}
function shouldRetry(obj) {
    return typeof obj === "object" && obj.message !== undefined;
}
async function promptText(prompt, textInput, resolvedDefault, enforceNonEmpty = false, converter) {
    ...
    if (textInput.text.validationRegex) {
        ...
        if (!userRe.test(res)) {
            ...
            return promptText(prompt, textInput, resolvedDefault, enforceNonEmpty, converter);
        }
    }
    if (enforceNonEmpty && res === "") {
        logger_1.logger.error(`A non-empty value is required, retrying......`);
        return promptText(prompt, textInput, resolvedDefault, enforceNonEmpty, converter);
    }
...
async function promptSelectMultiple(prompt, input, resolvedDefault, enforceNonEmpty = false, converter) {
    ...
    if (shouldRetry(converted)) {
        logger_1.logger.error(converted.message);
        return promptSelectMultiple(prompt, input, resolvedDefault, enforceNonEmpty, converter);
    }
